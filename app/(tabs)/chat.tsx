...
export default function ChatScreen() {
  const { activeChat, sendMessage, runProductionTool, decideBackendApproval, createChat, backendConnected, streamingReply, tools } = useNova();
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={[s.message, item.role === 'user' ? s.userMessage : s.assistantMessage, item.error && s.errorMessage]}>
              ...
              {item.approvalId && item.tool === 'web-search-summary' && <View style={s.approvalActions}><Text style={s.approvalHint}>No external URLs have been fetched.</Text><View style={s.approvalButtons}><Pressable style={s.approveButton} onPress={() => void decideBackendApproval(item.approvalId!, true)}><Text style={s.approveText}>Approve fetch</Text></Pressable><Pressable style={s.rejectButton} onPress={() => void decideBackendApproval(item.approvalId!, false)}><Text style={s.rejectText}>Reject</Text></Pressable></View></View>}
            </View>
          )}
