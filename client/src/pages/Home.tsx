...
export default function Home() {
  ...
  useEffect(() => { localStorage.setItem("nova-workspace-settings", JSON.stringify(workspaceSettings)); document.documentElement.style.setProperty("--nova-font-scale", `${workspaceSettings.fontScale}%`); document.documentElement.classList.toggle("nova-reduced-motion", workspaceSettings.reducedMotion); document.documentElement.classList.toggle("nova-high-contrast", workspaceSettings.highContrast); document.documentElement.dataset.novaTheme = workspaceSettings.theme; document.documentElement.style.setProperty("--nova-font-family", `\"${workspaceSettings.fontFamily}\", var(--font-sans)`); }, [workspaceSettings]);
  ...
  return (
    <div className={`app-shell ${workspaceSettings.compactComposer ? "compact-composer" : ""}`}>
      ...
      {sandboxOpen && <div className="overlay-scrim" onClick={() => setSandboxOpen(false)}><section className="sandbox-card" role="dialog" aria-label="Sandbox" onClick={(event) => event.stopPropagation()}><div className="modal-head"><div><span className="eyebrow">Bounded execution</span><h2>Sandbox</h2></div><IconButton label="Close sandbox" onClick={() => setSandboxOpen(false)}><X size={17} /></IconButton></div><p>Run short import-free calculations in an isolated, time-limited workspace.</p><textarea className="sandbox-editor" value={sandboxCode} onChange={(event) => setSandboxCode(event.target.value)} spellCheck={false} aria-label="Sandbox code" /><div className="sandbox-actions"><button className="primary-artifact-button" onClick={() => void runSandbox()} disabled={sandboxMutation.isPending}>{sandboxMutation.isPending ? "Running…" : "Run sandbox"}<Terminal size={14} /></button><span>5 s timeout · no filesystem · no network</span></div>{sandboxResult && <pre className="sandbox-result">{sandboxResult}</pre>}</section></div>}
