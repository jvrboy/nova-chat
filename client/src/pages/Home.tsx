...
type ThemeName = "paper" | "dark" | "midnight" | "sepia" | "mint";
type WorkspaceSettings = {
  enterToSend: boolean; showHints: boolean; autoSaveDrafts: boolean; compactDensity: boolean; reducedMotion: boolean; highContrast: boolean; fontScale: number; showTimestamps: boolean; showAvatars: boolean; spellcheck: boolean; smartCompose: boolean; citeSources: boolean; webSearchDefault: boolean; saveConversationHistory: boolean; localOnlyDrafts: boolean; allowAnalytics: boolean; notifyTaskComplete: boolean; notifyMentions: boolean; soundEffects: boolean; autoOpenArtifacts: boolean; artifactPreview: boolean; attachmentPreviews: boolean; maxAttachmentMb: number; redactAttachments: boolean; confirmExternalActions: boolean; keyboardShortcuts: boolean; language: string; timezone: string; theme: ThemeName;
};
const DEFAULT_WORKSPACE_SETTINGS: WorkspaceSettings = { enterToSend: true, showHints: true, autoSaveDrafts: true, compactDensity: false, reducedMotion: false, highContrast: false, fontScale: 100, showTimestamps: true, showAvatars: true, spellcheck: true, smartCompose: true, citeSources: true, webSearchDefault: false, saveConversationHistory: true, localOnlyDrafts: false, allowAnalytics: false, notifyTaskComplete: true, notifyMentions: true, soundEffects: false, autoOpenArtifacts: true, artifactPreview: true, attachmentPreviews: true, maxAttachmentMb: 25, redactAttachments: true, confirmExternalActions: true, keyboardShortcuts: true, language: "English", timezone: Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC", theme: "paper" };
...
export default function Home() {
  ...
  const providerStatusQuery = trpc.ai.providerStatus.useQuery(undefined, { refetchInterval: 5000, retry: 1 });
  ...
  const beginLogin = () => { try { startLogin(); } catch (error) { const message = error instanceof Error ? error.message : "Sign-in is unavailable for this deployment."; setErrorNotice(message); toast(message); } };
  ...
  useEffect(() => { localStorage.setItem("nova-workspace-settings", JSON.stringify(workspaceSettings)); document.documentElement.style.setProperty("--nova-font-scale", `${workspaceSettings.fontScale}%`); document.documentElement.classList.toggle("nova-reduced-motion", workspaceSettings.reducedMotion); document.documentElement.classList.toggle("nova-high-contrast", workspaceSettings.highContrast); document.documentElement.dataset.novaTheme = workspaceSettings.theme; }, [workspaceSettings]);
      ...
      <main className="chat-area">
        ...
        <div className="composer-dock">
          <div className="composer-wrap">
            ...
            <div className="composer-bottom"><div className="composer-context"><button className="model-selector" onClick={() => setModelOpen((value) => !value)}><Sparkles size={14} /><span>{selectedModel === "nova-fast" ? "Nova Fast" : "Nova 2"}</span><ChevronDown size={13} /></button>{modelOpen && <div className="model-menu"><div className="provider-menu-label">Connected backend AI</div><div className="provider-status-grid">{(providerStatusQuery.data ?? []).map((provider) => <div className="provider-status-row" key={provider.id}><span className={`provider-status-dot ${provider.configured && provider.availableKeyCount > 0 ? "online" : "offline"}`} /><span className="provider-status-copy"><strong>{provider.label}</strong><small>{provider.configured ? `${provider.availableKeyCount}/${provider.keyCount} keys available · ${provider.requests} requests · ${provider.tokensUsed.toLocaleString()} reported tokens` : "Not configured"}</small></span><span className="provider-status-state">{provider.configured ? (provider.availableKeyCount > 0 ? "Ready" : "Cooling") : "Off"}</span></div>)}</div><div className="provider-status-note">Quota remaining is shown only when a provider exposes it; API keys are never displayed.</div><button onClick={() => { setSelectedModel("nova-2"); setModelOpen(false); }}><span className="model-dot" /><span><strong>Nova 2</strong><small>Balanced and thoughtful</small></span>{selectedModel === "nova-2" && <Check size={15} />}</button><button onClick={() => { setSelectedModel("nova-fast"); setModelOpen(false); }}><span className="model-dot fast" /><span><strong>Nova Fast</strong><small>Quick everyday help</small></span>{selectedModel === "nova-fast" && <Check size={15} />}</button></div>}<span className="context-divider" /><button className="context-link" onClick={() => void runWebSearch()}><Globe2 size={14} />{searchMutation.isPending ? "Searching…" : "Web search"}</button></div><button className={`send-button ${draft.trim() ? "ready" : ""}`} onClick={sendMessage} aria-label="Send message"><Send size={17} /></button></div>
          </div>
