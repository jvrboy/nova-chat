...
function Sidebar({ open, onClose, selected, onSelect, onNew, items, projectCount, projects, activeProject, onCreateProject, onSelectProject, onToggleStar, onOpenPreferences, showHints, isAuthenticated }: { open: boolean; onClose: () => void; selected: number; onSelect: (id: number) => void; onNew: () => void; items: Conversation[]; projectCount: number; projects: Array<{ id: number; name: string }>; activeProject: number | null; onCreateProject: () => void; onSelectProject: (id: number | null) => void; onToggleStar: (id: number, starred: boolean) => void; onOpenPreferences: () => void; showHints: boolean; isAuthenticated: boolean }) {
    ...
    <>
      ...
      <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
        ...
        <div className="sidebar-footer">
          ...
          <button className="workspace-signin" onClick={() => { if (!isAuthenticated) startLogin(); else onOpenPreferences(); }}>{isAuthenticated ? "Account connected" : "Sign in to sync projects & chats"}<ArrowUpRight size={14} /></button>
        </div>
      </aside>
    </>
  );
...
export default function Home() {
  ...
  useEffect(() => { localStorage.setItem("nova-workspace-settings", JSON.stringify(workspaceSettings)); document.documentElement.style.setProperty("--nova-font-scale", `${workspaceSettings.fontScale}%`); document.documentElement.classList.toggle("nova-reduced-motion", workspaceSettings.reducedMotion); document.documentElement.classList.toggle("nova-high-contrast", workspaceSettings.highContrast); }, [workspaceSettings]);
  ...
  const toggleStar = async (id: number, isStarred: boolean) => {
    if (!isAuthenticated) { toast("Sign in to star persistent chats."); startLogin(); return; }
