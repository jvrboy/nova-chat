...
export default function Home() {
  ...
  const providersQuery = trpc.ai.providers.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  const connectionsQuery = trpc.ai.connections.useQuery(undefined, { enabled: isAuthenticated, retry: false });
  const [selectedProvider, setSelectedProvider] = useState<"gemini" | "groq" | "ollama-cloud" | "openrouter">("gemini");
  ...
  const sendMessage = async () => {
    ...
    try {
      const completion = await completeMutation.mutateAsync({ provider: selectedProvider, model: selectedModel, system: currentProject?.instructions ?? undefined, messages: [...messages.filter((message) => message.content.trim()).map((message) => ({ role: message.role, content: message.content })), { role: "user", content: value }] });
